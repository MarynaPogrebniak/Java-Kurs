...
public class hashSet_post_codes {
  ...
  public static void main(String[] args) {
    ...
    // эта структура сама заботится, об уникальности,
    ...
    HashSet<String> postCode = new HashSet<String>(); //инициализация HashSet
    ...
    do {
      ...
      userChoice = userInput.next().toLowerCase()
          .charAt(0); // имя_сканера.next().toLowerCase().charAt(0);
      if (userChoice == 's') {
        break;
      }
    } while (true);
    ...
    do {
      ...
      System.out.println(
          "Для для остановки проверки нажмите s, чтобы продолжить - любую другую букву.");
      userChoice = userInput.next().toLowerCase()
          .charAt(0); // имя_сканера.next().toLowerCase().charAt(0);
      if (userChoice == 's') {
        break;
